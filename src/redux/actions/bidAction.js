...
export const fetchTrans = () => async (dispatch) => {
  try {
    ...
    const fetchTransaction = await fetch(
      process.env.REACT_APP_ENDPOINT + '/api/v1/transaction',
    ...
    while (i < transaction.transaction.length) {
      ...
      const fetchImgDetail = await fetch(
        `${process.env.REACT_APP_ENDPOINT}/api/v1/products/picture/${transaction.transaction[i].barang.gambarbarangs[0].gambar}`,
        { method: 'GET', headers: { 'Content-Type': 'application/json' } },
      )
...
export const fetchTransId = (id) => async (dispatch) => {
  try {
    // console.log(id)
    const fetchTransaction = await fetch(
      process.env.REACT_APP_ENDPOINT + '/api/v1/transaction/' + id,
      { method: 'get', headers: { 'Content-Type': 'application/json' } },
    )
    ...
    // action komunikasi dengan backend untuk ambil detail gambar dari imagekit
    const fetchImgDetail = await fetch(
      `${process.env.REACT_APP_ENDPOINT}/api/v1/products/picture/${result.bid.bid.barang.gambarbarangs[0].gambar}`,
      { method: 'GET', headers: { 'Content-Type': 'application/json' } },
    )
    const imgDetail = await fetchImgDetail.json()
...
export const AcceptBid = (id) => async (dispatch) => {
  try {
    if (id != '') {
      const response = await fetch(
        process.env.REACT_APP_ENDPOINT + '/api/v1/transaction/product/' + id,
        {
          method: 'put',
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
