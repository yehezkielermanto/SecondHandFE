...
export const fetchTransId = (id) => async (dispatch) => {
  try {
    // console.log(id)
    const fetchTransaction = await fetch(
      REACT_APP_URLENDPOINT + '/api/v1/transaction/' + id,
      { method: 'get', headers: { 'Content-Type': 'application/json' } },
    )
    ...
    // action komunikasi dengan backend untuk ambil detail gambar dari imagekit
    const fetchImgDetail = await fetch(
      `${REACT_APP_URLENDPOINT}/api/v1/products/picture/${result.bid.bid.barang.gambarbarangs[0].gambar}`,
      { method: 'GET', headers: { 'Content-Type': 'application/json' } },
    )
    const imgDetail = await fetchImgDetail.json()
    ...
    const fetchImgDetailUser = await fetch(
      `${process.env.REACT_APP_URLENDPOINT}/api/v1/products/picture/${result.bid.bid.user.gambar}`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
...
export const AcceptBid = (id) => async (dispatch) => {
  try {
    if (id != '') {
      const response = await fetch(
        REACT_APP_URLENDPOINT + '/api/v1/transaction/product/' + id,
        {
          method: 'put',
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
